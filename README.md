...
| `feature/change-scene-runner`      | Change Scene runner | Separate `ChangeSceneRunner` with transparent subject cutout via `remove_background.py`, ComfyUI text-to-image background generation, final subject compositing, PromptBuilder support for scene prompts, cleanup of `ToolActionRunner`/`generation_tool_prompts`, and verification that existing runners (AI Enhancer, Glam Makeup, Remove Background, Remove Objects, Skin Improve, Smile) continued working after the routing refactor. | [README](https://github.com/amanzhola/mobile-assets-backend/blob/feature/change-scene-runner/README.md) |
