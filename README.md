...
| `feature/change-scene-runner`      | Change Scene runner | Extracted `change_scene` into a dedicated action runner with a new background-generation pipeline: transparent subject cutout via `remove_background.py`, ComfyUI text-to-image background generation, and final subject/background compositing. Also cleaned up legacy `ToolActionRunner` and verified existing runners remained unaffected after routing changes. | `ChangeSceneRunner`, `BuildChangeScenePrompt`, `BuildChangeSceneBackgroundWorkflow`, `change_scene_background.json`, `composite_subject_on_background.py` | [README](https://github.com/amanzhola/mobile-assets-backend/blob/feature/change-scene-runner/README.md) |
